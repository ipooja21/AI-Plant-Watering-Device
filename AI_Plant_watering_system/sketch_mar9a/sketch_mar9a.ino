...
bool autoMode = true; // AI Auto-watering mode enabled by default
int moistureThreshold = 500; // AI-predicted soil moisture threshold

String getSensorData() {
  int soilMoisture = analogRead(SOIL_MOISTURE_PIN);
  float temp = dht.readTemperature();
  float humidity = dht.readHumidity();
  String data = String(soilMoisture) + "," + String(temp) + "," + String(humidity) + "," + (autoMode ? "AUTO" : "MANUAL");
  return data;
}
...
void loop() {
  ...
  if (client) {
    ...
    if (request.indexOf("/WATER=ON") != -1 && !autoMode) {
      digitalWrite(RELAY_PIN, HIGH);
    }
    if (request.indexOf("/WATER=OFF") != -1 && !autoMode) {
      digitalWrite(RELAY_PIN, LOW);
    }
    ...
    client.println(".toggle { background-color: #007bff; color: white; }");
    ...
    client.println("<p><strong>Mode:</strong> <span id='mode'>AUTO</span></p>");
    ...
    client.println("<a href='/AUTO=ON'><button class='button toggle'>Auto Mode</button></a>");
    client.println("<a href='/AUTO=OFF'><button class='button toggle'>Manual Mode</button></a>");
    client.println("</div>");
    ...
    client.println("    document.getElementById('mode').innerText = values[3];");
    client.println("  });");
    client.println("}, 1000);");
    client.println("</script>");
